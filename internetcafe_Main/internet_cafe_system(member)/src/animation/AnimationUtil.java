...
public class AnimationUtil {
     ...
     public static void neonCardEntrance(Node card, String glowHex, double delaySeconds) {
        if (card == null) return;

        // Lightweight entrance: small translateY, no rotation, no DropShadow, no shimmer
        card.setOpacity(0);
        ...
        // Phase 1: slide up + fade in + subtle scale (0 -> 200ms)
        Timeline phase1 = new Timeline(
            new KeyFrame(Duration.ZERO,
                new KeyValue(card.opacityProperty(), 0, Interpolator.LINEAR),
                new KeyValue(card.translateYProperty(), 10, Interpolator.LINEAR),
                new KeyValue(card.scaleXProperty(), 0.96, Interpolator.LINEAR),
                new KeyValue(card.scaleYProperty(), 0.96, Interpolator.LINEAR)
            ),
            new KeyFrame(Duration.millis(200),
                new KeyValue(card.opacityProperty(), 1.0, Interpolator.EASE_OUT),
                new KeyValue(card.translateYProperty(), -3.0, Interpolator.SPLINE(0.22, 1.0, 0.36, 1.0)),
                new KeyValue(card.scaleXProperty(), 1.02, Interpolator.SPLINE(0.22, 1.0, 0.36, 1.0)),
                new KeyValue(card.scaleYProperty(), 1.02, Interpolator.SPLINE(0.22, 1.0, 0.36, 1.0))
            )
        );

        // Phase 2: gentle settle (200 -> 310ms)
        Timeline phase2 = new Timeline(
            new KeyFrame(Duration.ZERO,
                new KeyValue(card.translateYProperty(), -3.0, Interpolator.LINEAR),
                new KeyValue(card.scaleXProperty(), 1.02, Interpolator.LINEAR),
                new KeyValue(card.scaleYProperty(), 1.02, Interpolator.LINEAR)
            ),
        ...
        SequentialTransition entrance = new SequentialTransition(phase1, phase2);
        entrance.setDelay(Duration.seconds(delaySeconds));
    ...
    private static double paneWidth(Node node) {
        if (node instanceof Region r) {
            return Math.max(r.getWidth(), r.getPrefWidth() > 0 ? r.getPrefWidth() : 200);
        }
        return 200;
    }
